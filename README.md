...
The compiled files are in the [bin folder](https://github.com/victorporof/Tilt/tree/master/bin). Inside [build](https://github.com/victorporof/Tilt/tree/master/bin/build) you can find the unpacked [Tilt.xpi](https://github.com/victorporof/Tilt/raw/master/bin/Tilt.xpi) archive.

Tilt uses the Google Closure compiler to minify the Javascript files, with the --compilation_level ADVANCED_OPTIMIZATIONS flag. Therefore, [some JS externs](https://github.com/victorporof/Tilt/blob/master/bin/google-closure/tilt-externs.jsext) must be specified so important variable names are not renamed.
